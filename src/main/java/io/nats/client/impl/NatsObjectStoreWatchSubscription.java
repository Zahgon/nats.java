...
public class NatsObjectStoreWatchSubscription extends NatsWatchSubscription<ObjectInfo> {
    ...
    public NatsObjectStoreWatchSubscription(NatsObjectStore os, ObjectStoreWatcher watcher, ObjectStoreWatchOption... watchOptions) throws IOException, JetStreamApiException {
        ...
        finishInit(os, Collections.singletonList(os.rawAllMetaSubject()), deliverPolicy, headersOnly, ULONG_UNSET, getHandler(watcher, !ignoreDeletes), watcher.getConsumerNamePrefix());
    }

    @NonNull
    private static WatchMessageHandler<ObjectInfo> getHandler(ObjectStoreWatcher watcher, boolean includeDeletes) {
